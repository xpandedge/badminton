...
- **Two independent role axes (DELTA_SPEC D5):** group-membership role (`owner | member` — simplified from D5's 3-role model per squad UX decision) drives *permissions*; session `participantType` (`registered_user | guest`) drives *visibility*. Never conflate them. Permission checks live in `@picklebaddies/domain` pure predicates so the same logic runs in UI and server actions.
- **Any squad member can score/generate/rebalance.** `canEnterScore` / `canGenerateSchedule` / `canRebalance` return true for all members. Only `canManageSquad` / `canDeleteSession` are owner-only.
