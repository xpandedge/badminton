...
exports.rebalanceSession = void 0;
...
const locked_js_1 = require("./lib/locked.js");
 ...
 */
exports.rebalanceSession = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth)
    ...
    const result = await db.runTransaction(async (t) => {
        ...
        await (0, auth_js_1.requireGroupRole)(uid, session.groupId, domain_1.canGenerateSchedule, t);
        if (session.status !== "active" && session.status !== "paused") {
            throw new https_1.HttpsError("failed-precondition", "Session must be active or paused to rebalance.");
        }
        ...
        const matchesByRound = await Promise.all(roundsSnap.docs.map((rDoc) => t.get(db.collection(`sessions/${sessionId}/rounds/${rDoc.id}/matches`))));
        const allMatches = matchesByRound.flatMap((snap) => snap.docs.map((m) => ({ id: m.id, ref: m.ref, ...m.data() })));
        ...
        const recomputedStats = (0, locked_js_1.recomputeStatsFromLocked)(lockedFull, session.scoringMode);
        ...
        const engineInput = (0, mapping_js_1.mapSessionToEngineInput)(session, players, rounds, [], "rebalance");
        ...
        for (let i = 0; i < roundsSnap.docs.length; i++) {
            ...
            if (roundData.roundNumber <= elapsedRounds || roundData.status !== "scheduled")
                continue;
            for (const mDoc of matchesByRound[i].docs) {
                if (mDoc.data().status === "scheduled")
                    t.delete(mDoc.ref);
            }
            t.delete(rDoc.ref);
        }
        for (const sDoc of sitOutsSnap.docs) {
            if (sDoc.data().roundNumber > elapsedRounds)
                t.delete(sDoc.ref);
        }
        ...
        for (const match of engineOutput.matches) {
            if (match.roundNumber <= elapsedRounds)
                continue;
        ...
        for (const sitOut of engineOutput.sitOuts) {
            if (sitOut.roundNumber <= elapsedRounds)
                continue;
        ...
        const summary = (0, domain_1.buildRebalanceSummary)({
